...
public class Event<T> {
    ...
    public Event() {
        ...
        if (clazz != Event.class) {
            ...
            if (temp instanceof ParameterizedType) {
                type = ((ParameterizedType) temp).getActualTypeArguments()[0];
            }
        }
    }
     ...
     * <pre>{@code
     *   Flow text = ...
     *   Event<User> event = new Event<>(text);
     * }</pre>
     *
     ...
     *   Charset charset = ...
     *   try (InputStream text = ... ) {
     *      Event<User> event = new Event<>(text, charset);
     *   }
     * }</pre>
     *
     * @throws NullPointerException If the specified {@code text} is null
     ...
     * <pre>{@code
     *   byte[] text = ...
     *   Event<User> event = new Event<>(text, 0, 6);
     * }</pre>
     *
     * @throws NullPointerException      If the specified {@code text} is null
    ...
    public Event(
        ...
        this();
        flow = Flow.of(text, index, length);
    }
    ...
    /**
     * Returns the {@link Flow} of this event
     */
    @Nullable
    public Flow getFlow() {
        return flow;
     ...
     */
    public static final class Spoiler {
        /**
         * Unsafe and may be deleted
         */
