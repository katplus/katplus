        ...
        Map data = Spare.of(Map.class).down(
            Flow.of(
                "<?xml version=\"1.0\" encoding=\"UTF-8\" ?>\n" +
                    "<config>\n" +
                    "<name>kat</name>\n" +
                    "<link>https://kat.plus/kat</link>\n" +
                    "<name>plus</name>\n" +
                    "<link>https://kat.plus/plus</link>\n" +
                    "<name>kat.plus</name>\n" +
                    "<link>https://kat.plus/kat.plus</link>\n" +
                    "</config>"
            )
        );

        assertNotNull(data);
        assertEquals("{name=[kat, plus, kat.plus], link=[https://kat.plus/kat, https://kat.plus/plus, https://kat.plus/kat.plus]}", data.toString());
    }

    @Test
    @SuppressWarnings("rawtypes")
    public void test_parse3() throws IOException {
        Map data = Spare.of(Map.class).down(
            Flow.of(
                "<?xml version=\"1.0\" encoding=\"UTF-8\" ?>\n" +
                    "<channel>\n" +
        ...
        assertNotNull(data);
        assertNull(data.get("chan"));
        assertNull(data.get("alias"));
        assertEquals(1, data.get("id"));
        assertEquals(66.6, data.get("grade"));
        assertEquals("kat+", data.get("title"));
        assertEquals(false, data.get("blocked"));
        assertEquals("{id=1, chan=null, alias=null, title=kat+, grade=66.6, blocked=false, link=[https://kat.plus/, {href=https://kat.plus/}], item={id=1, title=<this is 'KAT' & \"+\">, link=https://kat.plus/MzE.html, view={value=∞, =99+}, author=kraity, content=this is data}}", data.toString());
    }
